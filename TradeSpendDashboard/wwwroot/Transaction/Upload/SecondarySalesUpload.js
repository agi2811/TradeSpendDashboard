...
var totalItems = 0;
var locked = null;
var _URL = baseUrl + "Upload";

var customStore = new DevExpress.data.CustomStore({
    key: "Id",
    load: function (loadOptions) {
        var loadUrl = _URL + "/GetDataSecondarySales?year=" + currentYear + "&month=" + currentMonth;
    ...
    byKey: function (key) {
        var d = new $.Deferred();
        $.get(baseUrl + "/GetDataSecondarySales?Id=" + key)
...
var optionYearsStore = new DevExpress.data.CustomStore({
    ...
    load: function (loadOptions) {
        ...
        var loadUrl = baseUrl + "Parameter/GetYearsSecondarySalesOption?transaction=Upload";
        return loadData(loadUrl, loadOptions);
    },
    byKey: function (key) {
        var d = new $.Deferred();
        $.get(baseUrl + "Parameter/GetYearsSecondarySalesOption?key=" + key + "&transaction=Upload")
            .done(function (dataItem) {
...
var optionMonthStore = new DevExpress.data.CustomStore({
    ...
    load: function (loadOptions) {
        var search = (loadOptions.searchValue !== null && loadOptions.searchValue !== "" ? loadOptions.searchValue : "");
        var loadUrl = baseUrl + "Parameter/GetMonthOption?isBudget=1";
        return loadData(loadUrl, loadOptions);
    },
    byKey: function (key) {
        var d = new $.Deferred();
        $.get(baseUrl + "Parameter/GetMonthOption?isBudget=1&key=" + key)
            .done(function (dataItem) {
                var data = dataItem.result;
...
function InitTableSecondarySales() {
    var now = new Date();
    ...
    dataGrid = $("#gridContainer").dxDataGrid({
        ...
        onToolbarPreparing: function (e) {
            ...
            e.toolbarOptions.items.unshift(
                ...
                },
                {
                    ...
                    onClick: function (e) {
                        if (locked == false) {
                            $("#formDownload").attr("action", "DownloadTemplateSecondarySales");
                ...
                },
                {
                    ...
                    onClick: function (e) {
                        if (locked == false) {
                            openModalUpload("SecondarySales");
                        }
                    },
                    template: function () {
                        var htm = "";
        ...
        onContentReady: function (e) {
            ...
            jQuery.ajax({
                method: 'POST',
                url: _URL + '/GetDataLockSecondarySales',
                data: {
                    year: currentYear,
                    month: currentMonth
                },
        ...
        onExporting: function (e) {
            ...
            var worksheet = workbook.addWorksheet('Secondary Sales Upload');
            DevExpress.excelExporter.exportDataGrid({
                ...
                workbook.xlsx.writeBuffer().then(function (buffer) {
                    saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'Export Secondary Sales Upload.xlsx');
                });
            });
            e.cancel = true;
        },
        ...
        columns: [
            ...
            { caption: "New Channel", dataField: "NewChannel", alignment: "left", width: 300, fixed: true },
            { caption: "Old Channel", dataField: "OldChannel", alignment: "left", width: 200, fixed: true },
            { caption: "Customer", dataField: "Customer", alignment: "left", width: 200, fixed: true },
            { caption: "Category", dataField: "Category", alignment: "left", width: 200, fixed: true },
            { caption: "Year", dataField: "Year", alignment: "center", width: 120, fixed: true },
            {
...
$(function () {
    InitTableSecondarySales();
})

function openModalUpload(flag) {
    $('#inpFlag').val(flag);
    $('#titleModalUpload').text('Secondary Sales Upload');
