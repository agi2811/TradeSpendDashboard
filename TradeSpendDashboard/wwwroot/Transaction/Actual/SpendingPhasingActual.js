...
var optionYearsStore = new DevExpress.data.CustomStore({
    ...
    load: function (loadOptions) {
        ...
        var loadUrl = baseUrl + "Parameter/GetYearsSpendingPhasingOption?transaction=Actual";
        return loadData(loadUrl, loadOptions);
    },
    byKey: function (key) {
        var d = new $.Deferred();
        $.get(baseUrl + "Parameter/GetYearsSpendingPhasingOption?key=" + key + "&transaction=Actual")
            .done(function (dataItem) {
...
function InitTableSpendingPhasing() {
    ...
    dataGrid = $("#gridContainer").dxDataGrid({
        ...
        onToolbarPreparing: function (e) {
            ...
            e.toolbarOptions.items.unshift(
                ...
                //},
                {
                    ...
                    onClick: function (e) {
                        if (locked == false) {
                            interfaceData("SpendingPhasing");
                        } 
                    },
                    template: function () {
                        var htm = "";
                        htm = "<button class='btn btn-success btn-download' title='Import From Trade Spend'><i class='fa fa-download'></i> Interface From Trade Spend</button>";
                        return $("<div/>")
                            .addClass("informer")
                            .append(htm);
                    }
        ...
        onExporting: function (e) {
            ...
            var worksheet = workbook.addWorksheet('Spending Phasing Actual');
            DevExpress.excelExporter.exportDataGrid({
                ...
                workbook.xlsx.writeBuffer().then(function (buffer) {
                    saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'Export Spending Phasing Actual.xlsx');
                });
            });
            e.cancel = true;
        },
...
//    $('#inpFlag').val(flag);
//    $('#titleModalUpload').text('Upload Spending Phasing');
...
function checkHasFailedUpload(flag) {
    var status = false;
    $.ajax({
        type: 'GET',
        url: _URL + '/HasFailed' + flag,
        async: false,
        processData: true,
        contentType: false,
        data: {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (flag === "SpendingPhasing") {
                var hasFailed = checkHasFailedUpload(flag);
                if (hasFailed) {
