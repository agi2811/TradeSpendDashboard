...
var optionYearsStore = new DevExpress.data.CustomStore({
    ...
    load: function (loadOptions) {
        ...
        var loadUrl = baseUrl + "Parameter/GetYearsSecondarySalesOption?transaction=Update";
        return loadData(loadUrl, loadOptions);
    },
    byKey: function (key) {
        var d = new $.Deferred();
        $.get(baseUrl + "Parameter/GetYearsSecondarySalesOption?key=" + key + "&transaction=Update")
            .done(function (dataItem) {
...
function InitTableSecondarySales() {
    ...
    dataGrid = $("#gridContainer").dxDataGrid({
                    ...
                    onClick: function (e) {
                        if (locked == false) {
                            if (dataGrid.getDataSource().totalCount() > 0) {
                                ...
                                Swal.fire('Warning', 'No data to Download!', 'warning');
                            }
                        }
                    },
                    template: function () {
        ...
        onExporting: function (e) {
            ...
            var worksheet = workbook.addWorksheet('Secondary Sales Update');
            DevExpress.excelExporter.exportDataGrid({
                ...
                workbook.xlsx.writeBuffer().then(function (buffer) {
                    saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'Export Secondary Sales Update.xlsx');
                });
            });
            e.cancel = true;
        },
...
function openModalUpload(flag) {
    $('#inpFlag').val(flag);
    $('#titleModalUpload').text('Secondary Sales Update');
