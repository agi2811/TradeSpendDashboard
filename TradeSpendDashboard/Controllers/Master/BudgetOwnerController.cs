...
{
    public class BudgetOwnerController : BaseController
    {
        private readonly ILogger<BudgetOwnerController> _logger;
        AppHelper _app;
        IMasterBudgetOwnerService _masterBudgetOwnerService;
        IGlobalService _globalService;
        public static string currentColumn = "";

        public BudgetOwnerController(
            ILogger<BudgetOwnerController> logger,
            AppHelper app,
            IWebHostEnvironment environment,
            IMasterMenuService masterMenu,
            IMasterBudgetOwnerService masterBudgetOwnerService,
            IGlobalService globalService
        ) : base(logger, app, environment, masterMenu)
        {
            _logger = logger;
            _app = app;
            _masterBudgetOwnerService = masterBudgetOwnerService;
        ...
        public IActionResult GetData()
        {
            try
            {
                ...
                var data = _masterBudgetOwnerService.GetAll();
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(MasterBudgetOwnerDTO model)
        {
            try
            {
                var cekData = await _masterBudgetOwnerService.GetByAllField(model.BudgetOwner);
                if (cekData.Any())
                {
                    if (model.Id != 0)
                    {
                        var updated = await _masterBudgetOwnerService.Update(model.Id, model);
                ...
                {
                    var result = await _masterBudgetOwnerService.Add(model);
        ...
        [HttpPost]
        public async Task<IActionResult> Update(long id, MasterBudgetOwnerDTO model)
        {
            try
            {
                var cekData = await _masterBudgetOwnerService.GetByAllField(model.BudgetOwner);
                ...
                if (!existId)
                {
                    var result = await _masterBudgetOwnerService.Update(id, model);
                    return Json(new { status = "success", result = result, message = "Success", });
                }
        ...
        public async Task<IActionResult> Delete(long id)
        {
            try
            {
                var result = await _masterBudgetOwnerService.Delete(id);
