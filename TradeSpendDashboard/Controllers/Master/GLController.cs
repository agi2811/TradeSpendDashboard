...
namespace TradeSpendDashboard.Controllers.Master
{
    public class GLController : BaseController
    {
        ...
        AppHelper _app;
        IMasterGLService _masterGLService;
        ...
        public GLController(
            ...
            IMasterGLService masterGLService,
            IGlobalService globalService
        ) : base(logger, app, environment, masterMenu)
        {
            _logger = logger;
            _app = app;
            _masterGLService = masterGLService;
        ...
        public IActionResult GetData()
        {
            try
            {
                ...
                var data = _masterGLService.GetAll();
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(MasterGLDTO model)
        {
            try
            {
                var cekData = await _masterGLService.GetByAllField(model.GLCode);
                if (cekData.Any())
                {
                    if (model.Id != 0)
                    {
                        var updated = await _masterGLService.Update(model.Id, model);
                ...
                {
                    var result = await _masterGLService.Add(model);
        ...
        [HttpPost]
        public async Task<IActionResult> Update(long id, MasterGLDTO model)
        {
            try
            {
                var cekData = await _masterGLService.GetByAllField(model.GLCode);
                ...
                if (!existId)
                {
                    var result = await _masterGLService.Update(id, model);
                    return Json(new { status = "success", result = result, message = "Success", });
                }
        ...
        public async Task<IActionResult> Delete(long id)
        {
            try
            {
                var result = await _masterGLService.Delete(id);
        ...
        public async Task<IActionResult> GetGLTypeOption(string search = "")
        {
            try
            {
                var data = await _masterGLService.GetGLTypeOption(search);
        ...
        public async Task<IActionResult> GetGLTypeOptionById(string search)
        {
            try
            {
                var data = await _masterGLService.GetGLTypeOptionById(search);
                return Ok(new { status = "success", result = data });
            }
            catch (Exception ex)
            {
